...
<body>
	<div id="container">
        ...
        <tbody>
			...
			$bids_stat = mysqli_query($conn,"SELECT * FROM bidreport LEFT JOIN member ON member.memberid = bidreport.bidder LEFT JOIN products ON products.productid = bidreport.productid WHERE bidreport.status = 0") or die(mysqli_error());
			WHILE($stat = mysqli_fetch_array($bids_stat)){
				echo 
					"<tr>
                        <td align='center'>".$stat['lastname'].", " .$stat['firstname']."Has Placed ".$stat['bidamount']." On Item ".$stat['prodname']."</td>
