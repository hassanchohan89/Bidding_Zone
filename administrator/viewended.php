...
<body>
	<div id="container">
        ...
        <tbody>
			...
			$bids_stat = mysqli_query($conn,"SELECT * FROM products WHERE status = 0") or die(mysqli_error());
			WHILE($stat = mysqli_fetch_array($bids_stat)){
				$prodid = $stat['productid'];
				$numbidderq = mysqli_query($conn,"Select * FROM bidreport WHERE productid = '$prodid'")or die(mysqli_error());
